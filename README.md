...
|**[bind()](overview/bind.md)**|Event Handling|`Q(selector).bind("click", () => console.log("Clicked"));`|Adds an event listener to each node with the ability to use event delegation.|
...
|**[closest()](overview/closest.md)**|Traversal|`Q(selector).closest(".parent");`|Returns the closest parent node of the first node that matches a specific selector.|
...
|**[hide()](overview/hide.md)**|Display|`Q(selector).hide(duration, callback);`|Hides each node.|
...
|**[inside()](overview/inside.md)**|Traversal|`Q(selector).inside(".parent");`|Checks if the first node is inside another node.|
...
|**[show()](overview/show.md)**|Display|`Q(selector).show(duration, callback);`|Shows each node.|
