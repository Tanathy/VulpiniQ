  ...
  Q.Image.prototype.Blur = function(options = {}) {
    ...
    // Apply convolution iterations for quality
  ...
  function createGaussianKernel(radius) {
    // Ensure radius is non-negative integer
    radius = Math.max(0, Math.floor(radius));
    const size = 2 * radius + 1;

    // If size is invalid (e.g., radius was negative initially), return identity
    ...
    // If radius is 0, return identity kernel directly
    if (radius === 0) {
      return { kernel: new Float32Array([1]), size: 1 };
    }

    const kernel = new Float32Array(size * size);
    const sigma = radius / 3; // Sigma based on radius
    const twoSigmaSquare = 2 * sigma * sigma;
    let sum = 0;
    const center = radius; // which is floor(size / 2)

    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        const dx = x - center;
        const dy = y - center;
        // Calculate weight using Gaussian formula
    ...
    if (sum <= 0 || !isFinite(sum)) {
      ...
      // fallback: make center pixel 1, rest 0
      kernel.fill(0);
      const centerIndex = center * size + center;
      if (centerIndex >= 0 && centerIndex < kernel.length) {
        kernel[centerIndex] = 1;
      } else {
        // If even center index is bad, return a minimal valid kernel
        return { kernel: new Float32Array([1]), size: 1 };
      }
      sum = 1; // Kernel now sums to 1
    } else {
      // Perform normalization only if sum is valid and positive
      for (let i = 0; i < kernel.length; i++) {
        kernel[i] /= sum;
        // Final check for NaN/Infinity after division
        if (!isFinite(kernel[i])) {
          console.warn("Gaussian kernel: Non-finite value after normalization, resetting to fallback 1x1 kernel.");
          kernel.fill(0);
          kernel[center * size + center] = 1; // Reset to identity
          break; // Exit loop
        }
      }
    }
