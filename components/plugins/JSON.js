...
// Variables: jsonData, data, key, newValue, callback, map, counter, obj, compressedData, replaceRecursive, level, deflatedJson, reverseMap, originalKey, value, restoreRecursive, inflatedData
Q.JSON = function (jsonData) {
    if (!(this instanceof Q.JSON)) {
        return new Q.JSON(jsonData);
    }
    this.json = jsonData;
};
